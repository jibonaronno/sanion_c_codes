...
/* After leafname is chopped at "$", the maximum size of each substring */
#define MAX_LEAF_NAME_SUB 150
#define MAX_LEAF_NAME_WHOLE 150
...
// diebu add - 2018.01.26.
typedef struct
{
	DBL_LNK l;
	ST_CHAR refdata[MAX_IDENT_LEN + 1];
	ST_UINT	index;
}SCL_SANION_PRI_DAIO;
...
typedef struct
{
	...
	ST_CHAR *desc;
	SCL_SANION_PRI_DAIO *pri_daioHead;
}SCL_SANION_PRI_DATASET;

// diebu add - 2018.01.26.
typedef struct
{
	DBL_LNK l;
  	ST_CHAR name[129+1];	/* Name of CB. Used to construct*/
					/* GoCBRef or GsCBRef		*/
  	ST_CHAR *desc;			/* description (optional)*/
					/* may be long so allocate if present*/
  	ST_CHAR datSet[129+1];	/* for GOOSE only	*/
					/* used to construct GOOSE DatSet*/
  	ST_UINT confRev;			/* for GOOSE only	*/
  	ST_BOOLEAN isGoose;	/* SD_TRUE if "GOOSE", SD_FALSE if "GSSE"*/
  	ST_CHAR appID[129+1];	/* for GOOSE only	*/
					/* maps to GoID in 61850-7-2	*/
	ST_UCHAR MAC[CLNP_MAX_LEN_MAC];	
	ST_UINT ApplID;	
	ST_UINT	TotNum;
	ST_CHAR SubdatSet[MAX_IDENT_LEN+1];
	ST_UCHAR PriArryType;
...
typedef struct
  ...
  // diebu add - 2018.01.26.
  SCL_SANION_PRI		  *sanion_priHead;
  SCL_SANION_PRI_DATASET  *sub_datasetHead;
...
typedef struct scl_dec_ctrl
  ...
  // diebu add - 2018.05.30.
  SCL_SANION_PRI *sanion_pri_gcb;
...
SCL_SANION_PRI *scl_sanion_pri_add(SCL_INFO *scl_info);
SCL_RCB *scl_rcb_add (
        SCL_INFO *scl_info);
...
ST_VOID GetLeafName(ST_CHAR* ps8leafname, ST_CHAR as8leafarr[][MAX_LEAF_NAME_SUB]);

ST_VOID* MatchRT(MVL_VAR_ASSOC* ptrBase, ST_CHAR as8leafarr[][MAX_LEAF_NAME_SUB], ST_INT s32totalidx);

MVL_DOM_CTRL* MatchDom(ST_CHAR* ps8lnname);

MVL_VAR_ASSOC* MatchVar(ST_CHAR* ps8lnname, MVL_DOM_CTRL* pstMVL_DOM);

ST_VOID* GetVmdAddr(ST_CHAR* ps8leafname);

ST_VOID PrintData(RUNTIME_TYPE* ptrRT, ST_VOID* ptrData);

ST_VOID* AdjustDataAddr(ST_VOID* ptrData, RUNTIME_TYPE* ptrRT, ST_BOOLEAN u8matched, ST_BOOLEAN u8samedeepness, ST_INT s32idx, ST_INT* s32Offset);

ST_INT GetDigit(ST_CHAR* ps8substring);

ST_CHAR* GetLeaf(ST_CHAR* ps8line);

ST_INT GetNumLeaf(FILE* fp);

ST_VOID CheckVmdArr(ST_INT* ps32addrarr, ST_INT s32numleaf);

void *safe_malloc(size_t n);

ST_INT IsTypeMatch(ST_CHAR* ps8line);

ST_INT IsLineValid(ST_CHAR* ps8line);

ST_CHAR* CBNameProcess(ST_CHAR* cbname);

//ST_VOID** GetVmdAddrForGivenLNClassAndCustomString(SCL_INFO *scl_info, ST_CHAR* ps8lnclass, ST_CHAR* ps8customstr, ST_UINT16* u16numofln);

ST_VOID* GetVmdAddrForGivenLNClassAndCustomString(SCL_INFO *scl_info, ST_CHAR* ps8lnclass, ST_CHAR* ps8customstr);

ST_CHAR** GetLeafNamesFromSCL(SCL_INFO *scl_info, leafParser***ptrLeafParsers, ST_INT16* ps16numLeafs);
